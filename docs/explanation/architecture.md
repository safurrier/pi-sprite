...
`/recap` retains the isolated no-tools helper in `src/agent/side-session.ts`: it creates an in-memory session with a safety prompt and disposes it after the response. BTW is deliberately different. On its first contextual request, `src/btw/session.ts` selects the exact active parent path as the child model context. The persistent child file may retain sibling history copied from the parent file, but those siblings remain outside the active model context. The child keeps its own transcript and identity while inheriting the active model/provider, thinking level, cwd, AGENTS context, loaded skill metadata, and `read`/`bash`/`edit`/`write` tools. Its resource loader deliberately excludes parent extensions so Pi Sprite and other lifecycle hooks are not recursively instantiated.

Parent progress is a deliberate transfer boundary: status is read-only, and `/btw:refresh` persists a bounded hidden snapshot into the child only when requested. Stream updates feed the existing BTW bubble, while the final answer is read only after the full `session.prompt()` lifecycle settles. The shared cwd means concurrent file mutation is possible; the BTW system prompt requires an explicit user request before mutation and explicit reporting afterward. Clear/new, branch changes, and shutdown cancel/dispose the runtime. The approach is adapted from [dbachelder/pi-btw](https://github.com/dbachelder/pi-btw).
