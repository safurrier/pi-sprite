...
`/recap` retains the isolated no-tools helper in `src/agent/side-session.ts`: it creates an in-memory session with a safety prompt and disposes it after the response. BTW is deliberately different. `src/btw/session.ts` extracts the exact active parent path into a persistent child session on its first contextual request. The child keeps its own transcript and identity while inheriting the active model/provider, thinking level, cwd, AGENTS context, loaded skill metadata, and `read`/`bash`/`edit`/`write` tools. Its resource loader deliberately excludes parent extensions so Pi Sprite and other lifecycle hooks are not recursively instantiated.
